 ...
 */
public class DefaultSingleTablePipeline extends AbstractHandlerBehaviour
    ...
    private KryoNamespace appKryo = new KryoNamespace.Builder()
            .register(KryoNamespaces.API).register(SingleGroup.class)
    ...
    private Cache<Integer, NextObjective> pendingAddNext = CacheBuilder
            .newBuilder().expireAfterWrite(20, TimeUnit.SECONDS)
            .removalListener((
                    RemovalNotification<Integer, NextObjective> notification) -> {
    ...
    public void filter(FilteringObjective filter) {
        ...
        case PERMIT:
            ...
            break;
        case DENY:
            actions = filter.meta() == null ? DefaultTrafficTreatment.builder()
                    : DefaultTrafficTreatment.builder(filter.meta());
            actions.drop();
            break;
        default:
        ...
        FlowRule.Builder ruleBuilder = DefaultFlowRule.builder()
                ...
                .withTreatment(actions.build()).fromApp(filter.appId())
        ...
        if (fwd.treatment() != null) {
            ...
            if (fwd.op() == ADD) {
                ...
                if (nextObjective == null) {
                    ...
                    // We verify that next was in the store and then
                    // de-serialize
                ...
                next = flowObjectiveStore.getNextGroup(fwd.nextId());
                treatment = next != null ? appKryo.deserialize(next.data())
                        : null;
            }
            // If the treatment is null we cannot re-build the original flow
            if (treatment == null) {
                fwd.context().ifPresent(
                        c -> c.onError(fwd, ObjectiveError.GROUPMISSING));
                return;
            }
            // Finally we build the flow rule and push to the flow rule
            // subsystem.
            FlowRule.Builder ruleBuilder = DefaultFlowRule.builder()
                    .forDevice(deviceId).withSelector(selector)
                    .fromApp(fwd.appId()).withPriority(fwd.priority())
    ...
    private void installObjective(FlowRule.Builder ruleBuilder,
            Objective objective) {
        ...
        FlowRuleOperations ops = flowBuilder
                .build(new FlowRuleOperationsContext() {
                    @Override
                    public void onSuccess(FlowRuleOperations ops) {
                        objective.context().ifPresent(
                                context -> context.onSuccess(objective));
                    }

                    @Override
                    public void onError(FlowRuleOperations ops) {
                        objective.context()
                                .ifPresent(context -> context.onError(objective,
                                        ObjectiveError.FLOWINSTALLATIONFAILED));
                    }
                });
        for (int i = 0; i < ops.stages().size(); i++) {
            for (FlowRuleOperation op : ops.stages().get(i)) {
                ProvHook.recordDerivation(op.rule(), objective);
            }
        }
    ...
    public void next(NextObjective nextObjective) {
        ...
        case ADD:
            ...
            if (treatment == null) {
                ...
                nextObjective.context().ifPresent(context -> context
                        .onError(nextObjective, ObjectiveError.UNSUPPORTED));
                return;
            }
            ...
            break;
        case REMOVE:
            NextGroup next = flowObjectiveStore
                    .removeNextGroup(nextObjective.id());
            if (next == null) {
                nextObjective.context().ifPresent(context -> context
                        .onError(nextObjective, ObjectiveError.GROUPMISSING));
                return;
            }
            break;
        default:
        ...
        nextObjective.context()
                .ifPresent(context -> context.onSuccess(nextObjective));
    }
    ...
    /**
     * Gets traffic treatment from a next objective. Merge traffic treatments
     * from next objective if the next objective is BROADCAST type and contains
     * multiple traffic treatments. Returns first treatment from next objective
     * if the next objective is SIMPLE type and it contains only one treatment.
     *
    ...
    private TrafficTreatment getTreatment(NextObjective nextObjective) {
        ...
        case SIMPLE:
            if (treatments.size() != 1) {
                log.error(
                        "Next Objectives of type SIMPLE should have only "
        ...
        default:
            log.error("Unsupported next objective type {}.",
                    nextObjective.type());
            return null;
        }
    }
