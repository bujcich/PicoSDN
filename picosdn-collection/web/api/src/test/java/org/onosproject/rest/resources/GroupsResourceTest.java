...
public class GroupsResourceTest extends ResourceTest {
    ...
    private static class MockGroup implements Group {
        ...
        public MockGroup(DeviceId deviceId, int appId, String appCookie,
                int id) {
    ...
    /**
     * Hamcrest matcher to check that a group representation in JSON matches the
     * actual group.
     */
    public static class GroupJsonMatcher extends TypeSafeMatcher<JsonObject> {
        ...
        public boolean matchesSafely(JsonObject jsonGroup) {
            ...
            if (group.buckets().buckets() != null) {
                final JsonArray jsonBuckets = jsonGroup.get("buckets")
                        .asArray();
                if (group.buckets().buckets().size() != jsonBuckets.size()) {
                    reason = "buckets array size of " + Integer
                            .toString(group.buckets().buckets().size());
                    return false;
                }
                for (final GroupBucket groupBucket : group.buckets()
                        .buckets()) {
                    boolean groupBucketFound = false;
                    for (int groupBucketIndex = 0; groupBucketIndex < jsonBuckets
                            .size(); groupBucketIndex++) {
                        final String jsonType = jsonBuckets
                                .get(groupBucketIndex).asObject().get("type")
                                .asString();
     ...
     * @return matcher
     */
    private static GroupJsonMatcher matchesGroup(Group group,
    ...
    public static class GroupJsonArrayMatcher
        ...
        public boolean matchesSafely(JsonArray json) {
            ...
            for (int jsonGroupIndex = 0; jsonGroupIndex < json
                    .size(); jsonGroupIndex++) {

                final JsonObject jsonGroup = json.get(jsonGroupIndex)
                        .asObject();
            ...
            if (!groupFound) {
                reason = "Group with id " + group.id().id().toString()
                        + " not found";
                return false;
            } else {
                return true;
    ...
    public void testGroupsEmptyArray() {
        expect(mockGroupService.getGroups(deviceId1)).andReturn(null)
                .anyTimes();
        expect(mockGroupService.getGroups(deviceId2)).andReturn(null)
                .anyTimes();
    ...
    public void testGroupsSingleDevice() {
        ...
        groups.add(group5);
        groups.add(group6);
        expect(mockGroupService.getGroups(anyObject())).andReturn(groups)
                .anyTimes();
        replay(mockGroupService);
        replay(mockDeviceService);
        final WebTarget wt = target();
        final String response = wt.path("groups/" + deviceId3).request()
                .get(String.class);
        final JsonObject result = Json.parse(response).asObject();
        assertThat(result, notNullValue());
    ...
    /**
     * Test the result of a rest api GET with specifying device id and
     * appcookie.
     */
    @Test
    ...
    public void testGroupByDeviceIdAndAppCookieNull() {
        ...
        replay(mockGroupService);
        final WebTarget wt = target();
        final Response response = wt.path("groups/" + deviceId3 + "/" + "0x222")
                .request().get();
    ...
    public void testPost() {
        ...
        assertThat(location,
                Matchers.startsWith("/groups/of:0000000000000001/"));
    }
    ...
    private JsonArray fetchAndCheckBuckets(DeviceId deviceId,
            int expectedBucketCount) {
        WebTarget wt = target();
        final String fetchGroupPath = wt.path("groups/" + deviceId).request()
                .get(String.class);
    ...
    private void removeBucketsViaRest(String endpointPath) {
        ...
        Response removeResponse = wt.path(endpointPath)
                .request(MediaType.APPLICATION_JSON_TYPE).delete();
        assertThat(removeResponse.getStatus(),
                is(HttpURLConnection.HTTP_NO_CONTENT));
    }
    ...
    class TestGroupService extends GroupServiceAdapter {
        ...
        public void removeBucketsFromGroup(DeviceId deviceId,
                GroupKey oldCookie, GroupBuckets buckets, GroupKey newCookie,
                ApplicationId appId) {
            if (!group1.buckets.buckets().isEmpty()) {
                ArrayList<GroupBucket> newList = new ArrayList<>(
                        group1.buckets.buckets());
    ...
    public void testAddRemoveBucket() {
        ...
        expect(mockGroupService.getGroups(anyObject()))
                .andDelegateTo(groupService).anyTimes();
        mockGroupService.addBucketsToGroup(anyObject(), anyObject(),
                anyObject(), anyObject(), anyObject());
        expectLastCall().andDelegateTo(groupService).anyTimes();
        mockGroupService.removeBucketsFromGroup(anyObject(), anyObject(),
                anyObject(), anyObject(), anyObject());
        expectLastCall().andDelegateTo(groupService).anyTimes();
        replay(mockGroupService);
        ...
        // Add buckets
        String addEndpointPath = String.format(endpointBasePath,
                group1.deviceId(), group1.appCookie());
        ...
        assertThat(addResponse.getStatus(),
                is(HttpURLConnection.HTTP_NO_CONTENT));

        // Check that buckets are there
        JsonArray bucketsAfterAdd = fetchAndCheckBuckets(deviceId1, 4);

        String bucketId1 = Long.toString(
                bucketsAfterAdd.get(0).asObject().get("bucketId").asLong());
        String bucketId2 = Long.toString(
                bucketsAfterAdd.get(1).asObject().get("bucketId").asLong());
        String bucketId3 = Long.toString(
                bucketsAfterAdd.get(2).asObject().get("bucketId").asLong());
        String bucketId4 = Long.toString(
                bucketsAfterAdd.get(3).asObject().get("bucketId").asLong());
        ...
        // Remove two buckets
        String removeTwoEndpointPath = addEndpointPath + bucketId2 + ','
                + bucketId3;
