...
public class IntentsResourceTest extends ResourceTest {
    ...
    private static final ApplicationId APP_ID = new DefaultApplicationId(1,
            "test");
    ...
    final ConnectPoint connectPoint1 = new ConnectPoint(deviceId1,
            PortNumber.portNumber(1L));
    final ConnectPoint connectPoint2 = new ConnectPoint(deviceId2,
            PortNumber.portNumber(1L));
    final ConnectPoint connectPoint3 = new ConnectPoint(deviceId2,
            PortNumber.portNumber(2L));
    final ConnectPoint connectPoint4 = new ConnectPoint(deviceId3,
            PortNumber.portNumber(1L));

    final TrafficTreatment treatment1 = DefaultTrafficTreatment.builder()
            .setEthDst(MacAddress.BROADCAST).build();
    final TrafficTreatment treatment2 = DefaultTrafficTreatment.builder()
            .setEthDst(MacAddress.IPV4_MULTICAST).build();

    final TrafficSelector selector1 = DefaultTrafficSelector.builder()
            .matchEthType((short) 3).matchIPProtocol((byte) 9).build();
    final TrafficSelector selector2 = DefaultTrafficSelector.builder()
            .matchEthType((short) 4).matchIPProtocol((byte) 10).build();

    final MockFlowEntry flow1 = new MockFlowEntry(deviceId1, 1, treatment1,
            selector1);
    final MockFlowEntry flow2 = new MockFlowEntry(deviceId1, 2, treatment2,
            selector2);

    final MockFlowRule flowRule1 = new MockFlowRule(deviceId1, 1, treatment1,
            selector1);
    final MockFlowRule flowRule2 = new MockFlowRule(deviceId1, 2, treatment2,
            selector2);
    ...
    private static class MockFlowEntry extends FlowEntryAdapter {
        ...
        public MockFlowEntry(DeviceId deviceId, long id,
                TrafficTreatment treatment, TrafficSelector selector) {
    ...
    public static class IntentJsonMatcher extends TypeSafeMatcher<JsonObject> {
        ...
        public boolean matchesSafely(JsonObject jsonIntent) {
            ...
            final JsonArray jsonResources = jsonIntent.get("resources")
                    .asArray();
            if (intent.resources() != null) {
                if (intent.resources().size() != jsonResources.size()) {
                    reason = "resources array size of "
                            + Integer.toString(intent.resources().size());
                    return false;
                }
                for (final NetworkResource resource : intent.resources()) {
                    ...
                    if (resource instanceof Link) {
                        ...
                        for (int resourceIndex = 0; resourceIndex < jsonResources
                                .size(); resourceIndex++) {
                            final ObjectNode value;
                            try {
                                value = (ObjectNode) codecContext.mapper()
                                        .readTree(jsonResources
                                                .get(resourceIndex).toString());
                            } catch (IOException e) {
                                reason = "bad json";
                                return false;
                            }
                            final Link link = codecContext.codec(Link.class)
                                    .decode(value, codecContext);
                        ...
                        for (int resourceIndex = 0; resourceIndex < jsonResources
                                .size(); resourceIndex++) {
                            final JsonValue value = jsonResources
                                    .get(resourceIndex);
     ...
     *
     * @param pathEntries   list of path conatining flow entries of a particular
     *                      intent
     * @param expectedAppId expected app id we are looking for
     * @return matcher
     */
    private static IntentStatsJsonMatcher matchesRelatedFlowEntries(
            List<List<FlowEntry>> pathEntries, final String expectedAppId) {
        return new IntentStatsJsonMatcher(pathEntries, expectedAppId);
    }

    /**
     * Hamcrest matcher to check that an list of flowEntries in JSON matches the
     * actual list of flow entries.
     */
    public static class IntentStatsJsonMatcher
        ...
        public IntentStatsJsonMatcher(final List<List<FlowEntry>> pathEntries,
        ...
        public boolean matchesSafely(JsonObject jsonIntent) {
            ...
            if (pathEntries != null) {
                ...
                for (List<FlowEntry> flowEntries : pathEntries) {
                    JsonArray jsonFlowEntries = jsonPaths.get(jsonPathIndex++)
                            .asArray();
                    ...
                    for (FlowEntry flow : flowEntries) {
                        ...
                        JsonObject jsonFlow = jsonFlowEntries
                                .get(jsonFlowEntryIndex++).asObject();
                        ...
                        // check device id
                        String jsonDeviceId = jsonFlow.get(DEVICE_ID)
                                .asString();
        ...
        private boolean checkFlowTreatment(FlowEntry flow,
                JsonObject jsonFlow) {

            if (flow.treatment() != null) {
                JsonObject jsonTreatment = jsonFlow.get(TREATMENT).asObject();
                JsonArray jsonInstructions = jsonTreatment.get(INSTRUCTIONS)
                        .asArray();

                if (flow.treatment().immediate().size() != jsonInstructions
                        .size()) {
                ...
                for (Instruction instruction : flow.treatment().immediate()) {
                    ...
                    for (int instructionIndex = 0; instructionIndex < jsonInstructions
                            .size(); instructionIndex++) {
                        String jsonType = jsonInstructions.get(instructionIndex)
                                .asObject().get(TYPE).asString();

                        String instructionType = instruction.type().name();
        ...
        private boolean checkFlowSelector(FlowEntry flow, JsonObject jsonFlow) {
            ...
            if (flow.selector() != null) {
                JsonObject jsonTreatment = jsonFlow.get(SELECTOR).asObject();

                JsonArray jsonCriteria = jsonTreatment.get(CRITERIA).asArray();

                if (flow.selector().criteria().size() != jsonCriteria.size()) {
                    reason = CRITERIA + " array size of " + Integer
                            .toString(flow.selector().criteria().size());
                    return false;
                }
                for (Criterion criterion : flow.selector().criteria()) {
                    boolean criterionFound = false;

                    for (int criterionIndex = 0; criterionIndex < jsonCriteria
                            .size(); criterionIndex++) {
                        String jsonType = jsonCriteria.get(criterionIndex)
                                .asObject().get(TYPE).asString();
    ...
    /**
     * Hamcrest matcher to check that an intent is represented properly in a
    ...
    public static class IntentJsonArrayMatcher
        ...
        public boolean matchesSafely(JsonArray json) {
            ...
            for (int jsonIntentIndex = 0; jsonIntentIndex < json
                    .size(); jsonIntentIndex++) {

                final JsonObject jsonIntent = json.get(jsonIntentIndex)
                        .asObject();
            ...
            if (!intentFound) {
                reason = "Intent with id " + intent.id().toString()
                        + " not found";
                return false;
            } else {
                return true;
    ...
    public void testIntentsSingle() {
        ...
        expect(mockIntentService.getIntent(Key.of(0, APP_ID))).andReturn(intent)
                .anyTimes();
        expect(mockIntentService.getIntent(Key.of("0", APP_ID)))
                .andReturn(intent).anyTimes();
        expect(mockIntentService.getIntent(Key.of(0, APP_ID))).andReturn(intent)
                .anyTimes();
        expect(mockIntentService.getIntent(Key.of("0x0", APP_ID)))
                .andReturn(null).anyTimes();
        replay(mockIntentService);
        expect(mockCoreService.getAppId(APP_ID.name())).andReturn(APP_ID)
                .anyTimes();
        replay(mockCoreService);
        final WebTarget wt = target();

        // Test get using key string
        final String response = wt.path("intents/" + APP_ID.name() + "/0")
                .request().get(String.class);
        final JsonObject result = Json.parse(response).asObject();
        assertThat(result, matchesIntent(intent));

        // Test get using numeric value
        final String responseNumeric = wt
                .path("intents/" + APP_ID.name() + "/0x0").request()
                .get(String.class);
    ...
    public void testRelatedFlowsForIntents() {
        ...
        FlowRuleIntent flowRuleIntent = new FlowRuleIntent(APP_ID, null,
                flowRules, new HashSet<NetworkResource>(),
        ...
        expect(mockIntentService.getInstallableIntents(intent.key()))
                .andReturn(installableIntents).anyTimes();
        replay(mockIntentService);

        expect(mockFlowService.getFlowEntries(deviceId1)).andReturn(flowEntries)
                .anyTimes();
        replay(mockFlowService);

        expect(mockCoreService.getAppId(APP_ID.name())).andReturn(APP_ID)
                .anyTimes();
        expect(mockCoreService.getAppId(APP_ID.id())).andReturn(APP_ID)
                .anyTimes();
        replay(mockCoreService);
        ...
        // Test get using key string
        final String response = wt
                .path("intents/relatedflows/" + APP_ID.name() + "/0").request()
                .get(String.class);
        final JsonObject result = Json.parse(response).asObject();
        ...
        // Test get using numeric value
        final String responseNumeric = wt
                .path("intents/relatedflows/" + APP_ID.name() + "/0x0")
                .request().get(String.class);
        final JsonObject resultNumeric = Json.parse(responseNumeric).asObject();
        assertThat(resultNumeric,
                matchesRelatedFlowEntries(paths, APP_ID.name()));
    }
    ...
    public void testIntentInstallables() {
        ...
        Link link1 = DefaultLink.builder().type(Link.Type.DIRECT)
                .providerId(ProviderId.NONE).src(connectPoint1)
                .dst(connectPoint2).build();

        Link link2 = DefaultLink.builder().type(Link.Type.DIRECT)
                .providerId(ProviderId.NONE).src(connectPoint3)
                .dst(connectPoint4).build();
        ...
        FlowRuleIntent flowRuleIntent = new FlowRuleIntent(APP_ID, null,
                new ArrayList<>(), resources, PathIntent.ProtectionType.PRIMARY,
                null);
        ...
        expect(mockIntentService.getIntent(Key.of(intentId, APP_ID)))
                .andReturn(intent).anyTimes();
        expect(mockIntentService
                .getIntent(Key.of(Long.toHexString(intentId), APP_ID)))
                        .andReturn(null).anyTimes();
        expect(mockIntentService.getInstallableIntents(intent.key()))
                .andReturn(installableIntents).anyTimes();
        replay(mockIntentService);
        ...
        // Test get using key string
        final String response = wt
                .path("intents/installables/" + APP_ID.name() + "/" + intentId)
                .request().get(String.class);
        final JsonObject result = Json.parse(response).asObject();
        assertThat(result.get(INSTALLABLES).asArray(),
                hasIntent(flowRuleIntent));

        // Test get using numeric value
        final String responseNumeric = wt
                .path("intents/installables/" + APP_ID.name() + "/"
                        + Long.toHexString(intentId))
                .request().get(String.class);
        final JsonObject resultNumeric = Json.parse(responseNumeric).asObject();
        assertThat(resultNumeric.get(INSTALLABLES).asArray(),
                hasIntent(flowRuleIntent));
    }
    ...
    public void testBadGet() {
        ...
        expect(mockIntentService.getIntent(Key.of(0, APP_ID))).andReturn(null)
                .anyTimes();
        replay(mockIntentService);
    ...
    public void testRemove() {
        ...
        expect(mockIntentService.getIntent(Key.of(2, appId))).andReturn(intent)
                .once();
    ...
    public void testBadRemove() {
        ...
        expect(mockIntentService.getIntent(Key.of(2, appId))).andReturn(null)
                .once();
        expect(mockIntentService.getIntent(Key.of("0x2", appId)))
                .andReturn(null).once();
    ...
    public void testIntentsMiniSummary() {
        ...
        final WebTarget wt = target();
        replay(mockIntentService);
        final String response = wt.path("intents/minisummary").request()
                .get(String.class);
        ...
        assertThat(jsonIntents, notNullValue());
        assertThat(jsonIntents.get("installed").toString(),
                containsString("2"));
    }
}
