...
@Service
public class MeterManager extends
        AbstractListenerProviderRegistry<MeterEvent, MeterListener, MeterProvider, MeterProviderService>
    ...
    public void modified(ComponentContext context) {
        ...
        defaultProvider.init(deviceService,
                createProviderService(defaultProvider), mastershipService,
                fallbackMeterPollFrequency);
    }
    ...
    @Override
    protected MeterProviderService createProviderService(
    ...
    public Meter submit(MeterRequest request) {
        ...
        Meter.Builder mBuilder = DefaultMeter.builder()
                .forDevice(request.deviceId()).fromApp(request.appId())
                .withBands(request.bands()).withId(id).withUnit(request.unit());
        ...
        store.storeMeter(m).whenComplete(
                (result, error) -> onComplete.accept(request, result, error));
        ProvHook.recordApiCall(ProvApiCallType.CREATE, m);
        return m;
    }
    ...
    public void withdraw(MeterRequest request, MeterId meterId) {
        Meter.Builder mBuilder = DefaultMeter.builder()
                .forDevice(request.deviceId()).fromApp(request.appId())
                .withBands(request.bands()).withId(meterId)
        ...
        store.deleteMeter(m).whenComplete(
                (result, error) -> onComplete.accept(request, result, error));
        ProvHook.recordApiCall(ProvApiCallType.DELETE, m);
    }

    @Override
    public Meter getMeter(DeviceId deviceId, MeterId id) {
        MeterKey key = MeterKey.key(deviceId, id);
        Meter tmp = store.getMeter(key);
    ...
    public Collection<Meter> getMeters(DeviceId deviceId) {
        List<Meter> tmp = store.getAllMeters().stream()
    ...
    public Collection<Meter> getAllMeters() {
        Collection<Meter> tmp = store.getAllMeters();
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    private class InternalMeterProviderService
        ...
        @Override
        public void pushMeterMetrics(DeviceId deviceId,
            ...
            // Look for meters defined in onos and missing in the device
            // (restore)
            allMeters.stream().forEach(m -> {
                if ((m.state().equals(MeterState.PENDING_ADD)
                    ...
                    log.debug("Adding meter missing in device {} {}", deviceId,
                            m);
            ...
            meterEntriesMap.entrySet().stream()
                    .filter(md -> !allMeters.stream()
                            .anyMatch(m -> m.id().equals(md.getKey())))
                    .forEach(mio -> {
                        Meter meter = mio.getValue();
                        // FIXME: Removing a meter is meaningfull for OpenFlow,
                        // but not for P4Runtime.
                        ...
                        if (meter.meterCellId().type() == MeterCellType.INDEX) {
                            ...
                            log.debug(
                                    "Remove meter in device not in onos {} {}",
                                    deviceId, mio.getKey());
                            provider().performMeterOperation(deviceId,
                                    new MeterOperation(meter,
            ...
            allMeters.forEach(m -> {
                if (m.state() == MeterState.PENDING_ADD) {
                    provider().performMeterOperation(m.deviceId(),
                            new MeterOperation(m, MeterOperation.Type.MODIFY));
        ...
        @Override
        public void pushMeterFeatures(DeviceId deviceId,
    ...
    private class InternalMeterStoreDelegate implements MeterStoreDelegate {
        ...
        public void notify(MeterEvent event) {
            ...
            switch (event.type()) {
            case METER_ADD_REQ:
                executorService.execute(new MeterInstaller(deviceId,
                        event.subject(), MeterOperation.Type.ADD));
                break;
            case METER_REM_REQ:
                executorService.execute(new MeterInstaller(deviceId,
                        event.subject(), MeterOperation.Type.REMOVE));
                break;
            case METER_ADDED:
                log.info("Meter added {}", event.subject());
                post(new MeterEvent(MeterEvent.Type.METER_ADDED,
                        event.subject()));
                break;
            case METER_REMOVED:
                log.info("Meter removed {}", event.subject());
                post(new MeterEvent(MeterEvent.Type.METER_REMOVED,
                        event.subject()));
                break;
            default:
    ...
    private class MeterInstaller implements Runnable {
        ...
        public MeterInstaller(DeviceId deviceId, Meter meter,
                MeterOperation.Type op) {
