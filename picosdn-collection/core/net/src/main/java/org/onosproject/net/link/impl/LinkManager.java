...
@Service
public class LinkManager extends
        AbstractListenerProviderRegistry<LinkEvent, LinkListener, LinkProvider, LinkProviderService>
    ...
    public Iterable<Link> getLinks() {
        checkPermission(LINK_READ);
        Iterable<Link> tmp = store.getLinks();
    ...
    public Iterable<Link> getActiveLinks() {
        checkPermission(LINK_READ);
        Iterable<Link> tmp = FluentIterable.from(getLinks())
    ...
    public Set<Link> getDeviceLinks(DeviceId deviceId) {
        ...
        Set<Link> tmp = Sets.union(store.getDeviceEgressLinks(deviceId),
    ...
    public Set<Link> getDeviceEgressLinks(DeviceId deviceId) {
        checkPermission(LINK_READ);
        checkNotNull(deviceId, DEVICE_ID_NULL);
        Set<Link> tmp = store.getDeviceEgressLinks(deviceId);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Set<Link> getDeviceIngressLinks(DeviceId deviceId) {
        checkPermission(LINK_READ);
        checkNotNull(deviceId, DEVICE_ID_NULL);
        Set<Link> tmp = store.getDeviceIngressLinks(deviceId);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Set<Link> getLinks(ConnectPoint connectPoint) {
        ...
        SetView<Link> tmp = Sets.union(store.getEgressLinks(connectPoint),
    ...
    public Set<Link> getEgressLinks(ConnectPoint connectPoint) {
        checkPermission(LINK_READ);
        checkNotNull(connectPoint, CONNECT_POINT_NULL);
        Set<Link> tmp = store.getEgressLinks(connectPoint);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Set<Link> getIngressLinks(ConnectPoint connectPoint) {
        checkPermission(LINK_READ);
        checkNotNull(connectPoint, CONNECT_POINT_NULL);
        Set<Link> tmp = store.getIngressLinks(connectPoint);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Link getLink(ConnectPoint src, ConnectPoint dst) {
        ...
        Link tmp = store.getLink(src, dst);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public void removeLinks(ConnectPoint connectPoint) {
        if (deviceService
                .getRole(connectPoint.deviceId()) != MastershipRole.MASTER) {
            return;
        }
    ...
    private class InternalLinkProviderService
        ...
        public void linkDetected(LinkDescription linkDescription) {
            ...
            if (linkDescription != null) {
                Link tmpPrev = store.getLink(linkDescription.src(),
                        linkDescription.dst());
                LinkEvent event = store.createOrUpdateLink(provider().id(),
                        linkDescription);
                Link tmpNew = store.getLink(linkDescription.src(),
                        linkDescription.dst());
        ...
        private LinkDescription validateLink(LinkDescription linkDescription) {
            BasicLinkConfig cfg = networkConfigService.getConfig(
                    linkKey(linkDescription.src(), linkDescription.dst()),
            ...
            BasicLinkConfig cfgRev = networkConfigService.getConfig(
                    linkKey(linkDescription.dst(), linkDescription.src()),
                    BasicLinkConfig.class);
    ...
    private void removeLinks(Set<Link> links, boolean isSoftRemove) {
        for (Link link : links) {
            ...
            if (event != null) {
                ...
                post(event);
                ProvHook.recordApiCall(ProvApiCallType.DELETE, link);
            }
        }
    }
    ...
    private class InternalNetworkConfigListener
        ...
        public boolean isRelevant(NetworkConfigEvent event) {
            return event.configClass().equals(BasicLinkConfig.class) && (event
        ...
        public void event(NetworkConfigEvent event) {
            ...
            if (!isAllowed(cfg)) {
                log.info("Kicking out links between {} and {}", lk.src(),
                        lk.dst());
        ...
        private void doUpdate(ConnectPoint src, ConnectPoint dst,
                BasicLinkConfig cfg) {
            ...
            ProviderId pid = Optional.ofNullable(link).map(Link::providerId)
                    .orElse(ProviderId.NONE);
            LinkEvent tmpE = store.createOrUpdateLink(pid, desc);
            Link tmp = tmpE.subject();
            ProvHook.recordApiCall(ProvApiCallType.UPDATE, link, tmp);
