...
public class EdgeManager
    ...
    private final Map<DeviceId, Set<ConnectPoint>> connectionPoints = Maps
            .newConcurrentMap();
    ...
    public boolean isEdgePoint(ConnectPoint point) {
        ...
        Set<ConnectPoint> connectPoints = connectionPoints
                .get(point.deviceId());
    ...
    public Iterable<ConnectPoint> getEdgePoints() {
        ...
        Iterable<ConnectPoint> tmp = builder.build();
    ...
    @Override
    public void emitPacket(ByteBuffer data,
            Optional<TrafficTreatment> treatment) {
        checkPermission(PACKET_WRITE);
        TrafficTreatment.Builder builder = treatment
                .map(DefaultTrafficTreatment::builder)
                .orElse(DefaultTrafficTreatment.builder());
        getEdgePoints()
                .forEach(p -> packetService.emit(packet(builder, p, data)));
    }
    ...
    public void emitPacket(DeviceId deviceId, ByteBuffer data,
        ...
        TrafficTreatment.Builder builder = treatment
                .map(DefaultTrafficTreatment::builder)
                .orElse(DefaultTrafficTreatment.builder());
        getEdgePoints(deviceId)
                .forEach(p -> packetService.emit(packet(builder, p, data)));
    }

    private OutboundPacket packet(TrafficTreatment.Builder builder,
            ConnectPoint point, ByteBuffer data) {
        builder.setOutput(point.port());
        return new DefaultOutboundPacket(point.deviceId(), builder.build(),
                data);
    }
    ...
    private void loadAllEdgePorts() {
        deviceService.getAvailableDevices().forEach(
                d -> deviceService.getPorts(d.id()).forEach(p -> addEdgePort(
                        new ConnectPoint(d.id(), p.number()))));
    }
    ...
    // Processes a device event by adding or removing its end-points in our
    // cache.
    private void processDeviceEvent(DeviceEvent event) {
        // FIXME handle the case where a device is suspended, this may or may
        // not come up
        DeviceEvent.Type type = event.type();
        DeviceId id = event.subject().id();

        if (type == DEVICE_ADDED || type == DEVICE_AVAILABILITY_CHANGED
                && deviceService.isAvailable(id)) {
            // When device is added or becomes available, add all its ports
            deviceService.getPorts(event.subject().id()).forEach(
                    p -> addEdgePort(new ConnectPoint(id, p.number())));
        } else if (type == DEVICE_REMOVED || type == DEVICE_AVAILABILITY_CHANGED
                && !deviceService.isAvailable(id)) {
            // When device is removed or becomes unavailable, remove all its
            // ports.
            // Note: cannot rely on Device subsystem, ports may be gone.
            Optional.ofNullable(connectionPoints.remove(id))
                    .orElse(ImmutableSet.of()).forEach(point -> post(
                            new EdgePortEvent(EDGE_PORT_REMOVED, point)));
    ...
    private boolean isEdgePort(ConnectPoint point) {
        ...
        return !point.port().isLogical() && deviceService.getPort(point) != null
    ...
    private void addEdgePort(ConnectPoint point) {
        if (isEdgePort(point)) {
            Set<ConnectPoint> set = connectionPoints.computeIfAbsent(
                    point.deviceId(), (k) -> Sets.newConcurrentHashSet());
            if (set.add(point)) {
                post(new EdgePortEvent(EDGE_PORT_ADDED, point));
                ProvHook.recordApiCall(ProvApiCallType.CREATE, point);
            }
        }
    }
    ...
    private void removeEdgePort(ConnectPoint point) {
        ...
        if (!point.port().isLogical()) {
            ...
            if (set.remove(point)) {
                ...
                ProvHook.recordApiCall(ProvApiCallType.DELETE, point);
            }
            if (set.isEmpty()) {
