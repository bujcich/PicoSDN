...
public class InterfaceManager
    ...
    private final Map<ConnectPoint, Set<Interface>> interfaces = Maps
            .newConcurrentMap();
    ...
    public void activate() {
        ...
        for (ConnectPoint subject : configService.getSubjects(SUBJECT_CLASS,
                CONFIG_CLASS)) {
            InterfaceConfig config = configService.getConfig(subject,
                    CONFIG_CLASS);
    ...
    public Set<Interface> getInterfaces() {
        Set<Interface> tmp = interfaces.values().stream()
    ...
    public Interface getInterfaceByName(ConnectPoint connectPoint,
            String name) {
        Optional<Interface> intf = interfaces
                .getOrDefault(connectPoint, Collections.emptySet()).stream()
                .filter(i -> i.name().equals(name)).findAny();

        Interface tmp = intf.orElse(null);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Set<Interface> getInterfacesByPort(ConnectPoint port) {
        ...
        Set<Interface> tmp = ImmutableSet.copyOf(intfs);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Interface getMatchingInterface(IpAddress ip) {
        Interface tmp = getMatchingInterfacesStream(ip).findFirst()
                .orElse(null);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Set<Interface> getMatchingInterfaces(IpAddress ip) {
        Set<Interface> tmp = getMatchingInterfacesStream(ip).collect(toSet());
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }

    private Stream<Interface> getMatchingInterfacesStream(IpAddress ip) {
        Stream<Interface> tmp = interfaces.values().stream()
                .flatMap(Collection::stream)
                .filter(intf -> intf.ipAddressesList().stream().anyMatch(
                        intfIp -> intfIp.subnetAddress().contains(ip)));
     ...
     * <p>
     * Only returns the first match if there are multiple untagged VLAN
     * configured on the connect point.
     *
    ...
    public VlanId getUntaggedVlanId(ConnectPoint connectPoint) {
        return getInterfacesByPort(connectPoint).stream()
                ...
                .map(Interface::vlanUntagged).findFirst().orElse(null);
    }

    /**
     * Returns tagged VLAN configured on given connect point.
     * <p>
     * Returns all matches if there are multiple tagged VLAN configured on the
     * connect point.
     *
     * @param connectPoint connect point
    ...
    public Set<VlanId> getTaggedVlanId(ConnectPoint connectPoint) {
        ...
        return interfaces.stream().map(Interface::vlanTagged)
    ...
    public VlanId getNativeVlanId(ConnectPoint connectPoint) {
        ...
        return interfaces.stream()
                ...
                .map(Interface::vlanNative).findFirst().orElse(null);
    }
        ...
        try {
            ...
            for (Interface intf : intfConfig.getInterfaces()) {
                if (intf.name().equals(Interface.NO_INTERFACE_NAME)) {
                    process(new InterfaceEvent(
                            InterfaceEvent.Type.INTERFACE_ADDED, intf));
                } else {
                    ...
                    if (oldIntf.isPresent()) {
                        ...
                        if (!oldIntf.get().equals(intf)) {
                            process(new InterfaceEvent(
                                    InterfaceEvent.Type.INTERFACE_UPDATED, intf,
                                    oldIntf.get()));
                        }
                    } else {
                        process(new InterfaceEvent(
            ...
            for (Interface intf : old) {
                if (!intf.name().equals(Interface.NO_INTERFACE_NAME)) {
                    process(new InterfaceEvent(
                            InterfaceEvent.Type.INTERFACE_REMOVED, intf));
                }
            }
    ...
    private Optional<Interface> findInterface(Interface intf,
            Set<Interface> set) {
    ...
    private void removeInterfaces(ConnectPoint port) {
        ...
        ProvHook.recordApiCall(ProvApiCallType.DELETE, old);

        old.stream().filter(i -> !i.name().equals(Interface.NO_INTERFACE_NAME))
                .forEach(i -> process(new InterfaceEvent(
                        InterfaceEvent.Type.INTERFACE_REMOVED, i)));
    }

    @Override
    public void add(Interface intf) {
        InterfaceConfig config = configService.addConfig(intf.connectPoint(),
                CONFIG_CLASS);

        config.addInterface(intf);
        ProvHook.recordApiCall(ProvApiCallType.CREATE, intf);

        configService.applyConfig(intf.connectPoint(), CONFIG_CLASS,
                config.node());
    }

    @Override
    public boolean remove(ConnectPoint connectPoint, String name) {
        InterfaceConfig config = configService.addConfig(connectPoint,
                CONFIG_CLASS);
        ...
        try {
            if (config.getInterfaces().isEmpty()) {
                ...
                configService.applyConfig(connectPoint, CONFIG_CLASS,
                        config.node());
            }
        } catch (ConfigException e) {
    ...
    private class InternalConfigListener implements NetworkConfigListener {
        ...
        public void event(NetworkConfigEvent event) {
            if (event.configClass() == CONFIG_CLASS) {
                ...
                case CONFIG_ADDED:
                case CONFIG_UPDATED:
                    event.config().ifPresent(config -> updateInterfaces(
