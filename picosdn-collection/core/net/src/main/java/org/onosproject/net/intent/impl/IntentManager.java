...
public class IntentManager
        extends AbstractListenerManager<IntentEvent, IntentListener> implements
        IntentService, IntentExtensionService, IntentInstallCoordinator {
    ...
    private static final EnumSet<IntentState> RECOMPILE = EnumSet
            .of(INSTALL_REQ, FAILED, WITHDRAW_REQ);
    private static final EnumSet<IntentState> WITHDRAW = EnumSet
            .of(WITHDRAW_REQ, WITHDRAWING, WITHDRAWN);

    private static final boolean DEFAULT_SKIP_RELEASE_RESOURCES_ON_WITHDRAWAL = false;
    @Property(name = "skipReleaseResourcesOnWithdrawal", boolValue = DEFAULT_SKIP_RELEASE_RESOURCES_ON_WITHDRAWAL, label = "Indicates whether skipping resource releases on withdrawal is enabled or not")
    ...
    public void activate() {
        ...
        batchExecutor = newSingleThreadExecutor(
                groupedThreads("onos/intent", "batch", log));
        workerExecutor = newFixedThreadPool(numThreads,
                groupedThreads("onos/intent", "worker-%d", log));
    ...
    public void modified(ComponentContext context) {
        ...
        String s = Tools.get(context.getProperties(),
                "skipReleaseResourcesOnWithdrawal");
        boolean newTestEnabled = isNullOrEmpty(s)
                ? skipReleaseResourcesOnWithdrawal
    ...
    private void logConfig(String prefix) {
        log.info("{} with skipReleaseResourcesOnWithdrawal = {}", prefix,
                skipReleaseResourcesOnWithdrawal);
    }
    ...
    public void submit(Intent intent) {
        ...
        ProvHook.recordApiCall(ProvApiCallType.CREATE, intent);
    }
    ...
    public void withdraw(Intent intent) {
        ...
        store.addPending(data);
        ProvHook.recordApiCall(ProvApiCallType.DELETE, intent);
    }
    ...
    public void purge(Intent intent) {
        ...
        if (intent instanceof PointToPointIntent) {
            ...
            DeviceId deviceId = pointIntent.filteredIngressPoint()
                    .connectPoint().deviceId();
            GroupKey groupKey = PointToPointIntentCompiler
    ...
    public Intent getIntent(Key key) {
        ...
        Intent tmp = store.getIntent(key);
    ...
    public Iterable<Intent> getIntents() {
        checkPermission(INTENT_READ);
        Iterable<Intent> tmp = store.getIntents();
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public void addPending(IntentData intentData) {
        ...
        ProvHook.recordApiCall(ProvApiCallType.CREATE, intentData.intent());
    }

    @Override
    public Iterable<IntentData> getIntentData() {
        checkPermission(INTENT_READ);
        Iterable<IntentData> tmp = store.getIntentData(false, 0);
        for (IntentData intent : tmp) {
            ProvHook.recordApiCall(ProvApiCallType.READ, intent.intent());
        }
        return tmp;
    }
    ...
    public List<Intent> getInstallableIntents(Key intentKey) {
        checkPermission(INTENT_READ);
        checkNotNull(intentKey, INTENT_ID_NULL);
        List<Intent> tmp = store.getInstallableIntents(intentKey);
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    public Iterable<Intent> getPending() {
        checkPermission(INTENT_READ);
        Iterable<Intent> tmp = store.getPending();
        ProvHook.recordApiCall(ProvApiCallType.READ, tmp);
        return tmp;
    }
    ...
    private class InternalStoreDelegate implements IntentStoreDelegate {
        ...
        private void releaseResources(Intent intent) {
            // If a resource group is set on the intent, the resource consumer
            // is
            // set equal to it. Otherwise it's set to the intent key
            ResourceConsumer resourceConsumer = intent.resourceGroup() != null
            ...
            if (intent.resourceGroup() == null) {
                ...
                // related resources.
                Long remainingIntents = Tools.stream(store.getIntents())
                        .filter(i -> {
                            return i.resourceGroup() != null
                                    && i.resourceGroup()
                                            .equals(intent.resourceGroup());
                        }).count();
            ...
            if (removeResource) {
                ...
                if (!resourceService.release(resourceConsumer)) {
                    log.error("Failed to release resources allocated to {}",
                            resourceConsumer);
                }
            }
        }
    ...
    private class InternalTopoChangeDelegate implements TopologyChangeDelegate {
        ...
        public void triggerCompile(Iterable<Key> intentKeys,
            ...
            log.trace("submitting {} + all?:{} for compilation", intentKeys,
                    compileAllFailed);
    ...
    private class InternalBatchDelegate implements IntentBatchDelegate {
        ...
        public void execute(Collection<IntentData> operations) {
            ...
            // batchExecutor is single-threaded, so only one batch is in flight
            // at a time
            CompletableFuture.runAsync(() -> {
                // process intent until the phase reaches one of the final
                // phases
                List<CompletableFuture<IntentData>> futures = operations
                        .stream().map(data -> {
                            log.debug("Start processing of {} {}@{}",
                                    data.request(), data.key(), data.version());
                            return data;
                        })
                        .map(x -> CompletableFuture.completedFuture(x)
                                .thenApply(
                                        IntentManager.this::createInitialPhase)
                                .thenApplyAsync(IntentProcessPhase::process,
                                        workerExecutor)
                                .thenApply(FinalIntentProcessPhase::data)
                                .exceptionally(e -> {
                                    // When the future fails, we update the
                                    // Intent to simulate the failure of
                                    // the installation/withdrawal phase and we
                                    // save in the current map. In
                                    // the next round the CleanUp Thread will
                                    // pick this Intent again.
                                    ...
                                    case WITHDRAW_REQ:
                                    case WITHDRAWING:
                                        // TODO should we swtich based on
                                        // current
                                        IntentData current = store
                                                .getIntentData(x.key());
                                        return IntentData.nextState(current,
                                                FAILED);
                                    default:
                                        return null;
                                    }
    ...
    private IntentProcessPhase createInitialPhase(IntentData data) {
        ...
        if (pending == null || pending.version().isNewerThan(data.version())) {
             ...
             * previous batch iteration, so we can skip it. If the pending map
             * has a newer request, it will get compiled as part of the next
             * batch, so we can skip it.
             */
