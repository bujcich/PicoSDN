...
public class FlowObjectiveManager implements FlowObjectiveService {
    ...
    private final Map<DeviceId, DriverHandler> driverHandlers = Maps
            .newConcurrentMap();
    protected final Map<DeviceId, Pipeliner> pipeliners = Maps
            .newConcurrentMap();
    ...
    final Map<Integer, Set<PendingFlowObjective>> pendingForwards = Maps
            .newConcurrentMap();
    final Map<Integer, List<PendingFlowObjective>> pendingNexts = Maps
            .newConcurrentMap();
    ...
    protected void modified(ComponentContext context) {
        ...
        int newNumThreads = isNullOrEmpty(propertyValue) ? numThreads
        ...
        public void run() {
            try {
                ...
                if (pipeliner != null) {
                    ...
                    executorService.execute(new ObjectiveInstaller(deviceId,
                            objective, numAttempts + 1));
                } else {
                    // Otherwise we've tried a few times and failed, report an
                    // error back to the user.
                    objective.context().ifPresent(c -> c.onError(objective,
                            ObjectiveError.NOPIPELINER));
                }
                // Exception thrown
            } catch (Exception e) {
    ...
    public void filter(DeviceId deviceId,
        ...
        ProvHook.recordApiCall(ProvApiCallType.CREATE, filteringObjective);
        executorService
                .execute(new ObjectiveInstaller(deviceId, filteringObjective));
    }
    ...
    public void forward(DeviceId deviceId,
        ...
        checkPermission(FLOWRULE_WRITE);
        ProvHook.recordApiCall(ProvApiCallType.CREATE, forwardingObjective);
        if (forwardingObjective.nextId() == null
                || flowObjectiveStore
                        .getNextGroup(forwardingObjective.nextId()) != null
                || !queueFwdObjective(deviceId, forwardingObjective)) {
            // fast path
            executorService.execute(
                    new ObjectiveInstaller(deviceId, forwardingObjective));
        }
    }
    ...
    public void next(DeviceId deviceId, NextObjective nextObjective) {
        checkPermission(FLOWRULE_WRITE);
        ProvHook.recordApiCall(ProvApiCallType.CREATE, nextObjective);
        if (nextObjective.op() == Operation.ADD
            ...
            // either group exists or we are trying to create it - let it
            // through
            executorService
    ...
    boolean queueFwdObjective(DeviceId deviceId, ForwardingObjective fwd) {
        ...
        synchronized (pendingForwards) {
            // double check the flow objective store, because this block could
            // run
            ...
            if (flowObjectiveStore.getNextGroup(fwd.nextId()) == null) {
                pendingForwards.compute(fwd.nextId(), (id, pending) -> {
                    PendingFlowObjective pendfo = new PendingFlowObjective(
                            deviceId, fwd);
                    if (pending == null) {
        ...
        if (queued) {
            log.debug(
                    "Queued forwarding objective {} for nextId {} meant for device {}",
    ...
    boolean queueNextObjective(DeviceId deviceId, NextObjective next) {
        ...
        if (queued) {
            log.debug(
                    "Queued next objective {} with operation {} meant for device {}",
     ...
     * Otherwise it warms the caches and triggers the init method of the
     * Pipeline.
     *
     ...
     * and triggers the init method of the pipeline. Otherwise (DEVICE_ADDED) it
     * warms the caches and triggers the init method of the Pipeline. The
     * rationale of this method is for managing the scenario of a switch that
     * goes down for a failure and goes up after a while.
     *
     * @param deviceId the id of the device associated to the pipeline
     * @return the implementation of the Pipeliner behaviour
     */
     ...
     * <p>
     * Note: Expected to be called under per-Device lock. e.g.,
     * {@code pipeliners}' Map#compute family methods
     *
    ...
    private Pipeliner initPipelineHandler(DeviceId deviceId) {
        ...
        if (handler == null) {
            try {
                // Otherwise create it and if it has pipeline behaviour, cache
                // it
    ...
    private void invalidatePipelinerIfNecessary(Device device) {
        ...
        if (handler != null && !Objects.equals(handler.driver().name(),
        ...
        public void event(DeviceEvent event) {
            ...
            case DEVICE_AVAILABILITY_CHANGED:
                ...
                if (deviceService.isAvailable(event.subject().id())) {
                    log.debug("Device is now available {}",
                            event.subject().id());
                    getAndInitDevicePipeliner(event.subject().id());
                } else {
                    log.debug("Device is no longer available {}",
                            event.subject().id());
                }
                break;
            case DEVICE_UPDATED:
    ...
    // Monitors driver configuration changes and invalidates the pipeliner cache
    // entries.
    // Note that this may leave stale entries on the device if the driver
    // changes
    // in manner where the new driver does not produce backward compatible flow
    // objectives.
    // In such cases, it is the operator's responsibility to force device
    // re-connect.
    ...
    private void stopWatch() {
        ...
        if (duration > LIMIT) {
            log.info(
                    "Pipeline setup took {} ms; avg {} ms; cTime={}, dTime={}, eTime={}, hTime={}, hbTime={}",
                    duration, totals / count, diff(cTime), diff(dTime),
                    diff(eTime), diff(hTime), diff(hbTime));
        }
    }
        ...
        public void notify(ObjectiveEvent event) {
            if (event.type() == Type.ADD) {
                ...
                if (pending == null) {
                    ...
                    log.debug(
                            "Processing {} pending forwarding objectives for nextId {}",
                ...
                if (pendNexts == null) {
                    ...
                    log.debug(
                            "Processing {} pending next objectives for nextId {}",
    ...
    /**
     * Data class used to hold a pending flow objective that could not be
     * processed because the associated next object was not present. Note that
     * this pending flow objective could be a forwarding objective waiting for a
     * next objective to complete execution. Or it could a next objective (with
     * a different operation - remove, addToExisting, or removeFromExisting)
     * waiting for a next objective with the same id to complete execution.
     */
    ...
    public List<String> getNextMappings() {
        ...
        // XXX if the NextGroup after de-serialization actually stored info of
        // the deviceId
        // then info on any nextObj could be retrieved from one controller
        // instance.
        // Right now the drivers on one instance can only fetch for next-ids
        // that came
        // to them.
        // Also, we still need to send the right next-id to the right driver as
        // potentially
        // there can be different drivers for different devices. But on that
        // account,
        ...
        for (Map.Entry<Integer, NextGroup> e : allnexts.entrySet()) {
            ...
            mappings.add(
                    "NextId " + e.getKey() + ": " + (deviceId != null ? deviceId
                            : "nextId not in this onos instance"));
            if (deviceId != null) {
                // this instance of the controller sent the nextObj to a driver
                Pipeliner pipeliner = getDevicePipeliner(deviceId);
                List<String> nextMappings = pipeliner
                        .getNextMappings(e.getValue());
    ...
    public List<String> getPendingFlowObjectives() {
        ...
        for (Integer nextId : pendingForwards.keySet()) {
            ...
            for (PendingFlowObjective pf : pfwd) {
                pend.append("\n    FwdId: ")
                        ...
                        .append(", DeviceId: ").append(pf.deviceId())
                        .append(", Selector: ")
        ...
        for (Integer nextId : pendingNexts.keySet()) {
            ...
            for (PendingFlowObjective pn : pnext) {
                pend.append("\n    NextOp: ").append(pn.flowObjective().op())
                        .append(", DeviceId: ").append(pn.deviceId())
                        .append(", Treatments: ")
                        .append(((NextObjective) pn.flowObjective()).next());
            }
            pendingFlowObjectives.add(pend.toString());
        }
