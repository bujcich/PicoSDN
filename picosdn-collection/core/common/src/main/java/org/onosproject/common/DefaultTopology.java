...
public class DefaultTopology extends AbstractModel implements Topology {
     ...
     */
    public static synchronized void setDefaultLinkWeigher(
            LinkWeigher linkWeigher) {
    ...
    public static synchronized void setDefaultGraphPathSearch(
        ...
        log.info("Setting new default graph path algorithm to {}",
                graphPathSearch);
    ...
    public boolean isBroadcastPoint(ConnectPoint connectPoint) {
        ...
        TopologyCluster cluster = clustersByDevice()
                .get(connectPoint.deviceId());
        checkArgument(cluster != null, "No cluster found for device %s",
     ...
     * maintaining any ordering guarantees provided by the underlying (default
     * or if no default is specified {@link DijkstraGraphSearch}) search. If
     * returning all paths of a given length would exceed {@code maxPaths} a
     * subset of paths of that length will be returned, which paths will be
     * returned depends on the currently specified {@code GraphPathSearch}. See
     * {@link #setDefaultGraphPathSearch}.
     *
     * @param src      source device
     * @param dst      destination device
     * @param weigher  link weight function
    ...
    public Set<Path> getPaths(DeviceId src, DeviceId dst, LinkWeigher weigher,
        ...
        GraphPathSearch.Result<TopologyVertex, TopologyEdge> result = graphPathSearch()
                .search(graph, srcV, dstV, weigher, maxPaths);
        ImmutableSet.Builder<Path> builder = ImmutableSet.builder();
        for (org.onlab.graph.Path<TopologyVertex, TopologyEdge> path : result
                .paths()) {
    ...
    /**
     * Computes on-demand the k-shortest paths between source and destination
     * devices.
     *
     * @param src      source device
     * @param dst      destination device
     ...
     * devices.
     *
     * The first {@code maxPaths} paths will be returned in ascending order
     * according to the provided {@code weigher}
     *
     * @param src      source device
     * @param dst      destination device
     * @param weigher  link weight function
    ...
    public Set<Path> getKShortestPaths(DeviceId src, DeviceId dst,
        ...
        return KSHORTEST.search(graph, srcV, dstV, weigher, maxPaths).paths()
                .stream().map(this::networkPath)
    ...
    /**
     * Computes on-demand the set of shortest disjoint path pairs between source
    ...
    public Set<DisjointPath> getDisjointPaths(DeviceId src, DeviceId dst,
        ...
        GraphPathSearch.Result<TopologyVertex, TopologyEdge> result = SUURBALLE
                .search(graph, srcV, dstV, weigher, ALL_PATHS);
        ImmutableSet.Builder<DisjointPath> builder = ImmutableSet.builder();
        for (org.onlab.graph.Path<TopologyVertex, TopologyEdge> path : result
                .paths()) {
            DisjointPath disjointPath = networkDisjointPath(
    ...
    private Set<DisjointPath> disjointPaths(DeviceId src, DeviceId dst,
            LinkWeigher weigher, Map<TopologyEdge, Object> riskProfile) {
        DefaultTopologyVertex srcV = new DefaultTopologyVertex(src);
        DefaultTopologyVertex dstV = new DefaultTopologyVertex(dst);
        ...
        SrlgGraphSearch<TopologyVertex, TopologyEdge> srlg = new SrlgGraphSearch<>(
                riskProfile);
        GraphPathSearch.Result<TopologyVertex, TopologyEdge> result = srlg
                .search(graph, srcV, dstV, weigher, ALL_PATHS);
        ImmutableSet.Builder<DisjointPath> builder = ImmutableSet.builder();
        for (org.onlab.graph.Path<TopologyVertex, TopologyEdge> path : result
                .paths()) {
    ...
    private Path networkPath(
            org.onlab.graph.Path<TopologyVertex, TopologyEdge> path) {
    ...
    private DisjointPath networkDisjointPath(
        ...
        if (!path.hasBackup()) {
            ...
            return new DefaultDisjointPath(CORE_PROVIDER_ID,
                    (DefaultPath) networkPath(path.primary()), null);
        }
        return new DefaultDisjointPath(CORE_PROVIDER_ID,
    ...
    private ImmutableMap<ClusterId, TopologyCluster> buildTopologyClusters() {
        ImmutableMap.Builder<ClusterId, TopologyCluster> clusterBuilder = ImmutableMap
                .builder();
        ...
        for (int i = 0, n = results.clusterCount(); i < n; i++) {
            ...
            DefaultTopologyCluster cluster = new DefaultTopologyCluster(cid,
                    vertexSet.size(), edgeSet.size(), findRoot(vertexSet));
    ...
    private TopologyVertex findRoot(Set<TopologyVertex> vertexSet) {
        ...
        for (TopologyVertex vertex : vertexSet) {
            if (minVertex == null || vertex.deviceId().toString()
    ...
    private void addClusterBroadcastSet(TopologyCluster cluster,
        ...
        Result<TopologyVertex, TopologyEdge> result = DIJKSTRA.search(graph,
                cluster.root(), null, hopCountWeigher, 1);
        for (Map.Entry<TopologyVertex, Set<TopologyEdge>> entry : result
                .parents().entrySet()) {
    ...
    private ClusterIndexes buildIndexes() {
        ...
        ImmutableMap.Builder<DeviceId, TopologyCluster> clusterBuilder = ImmutableMap
                .builder();
        ImmutableSetMultimap.Builder<TopologyCluster, DeviceId> devicesBuilder = ImmutableSetMultimap
                .builder();
        ImmutableSetMultimap.Builder<TopologyCluster, Link> linksBuilder = ImmutableSetMultimap
                .builder();
        ...
        for (TopologyCluster cluster : clusters.get().values()) {
            ...
            for (TopologyVertex vertex : clusterResults.get().clusterVertexes()
                    .get(i)) {
            ...
            for (TopologyEdge edge : clusterResults.get().clusterEdges()
                    .get(i)) {
    ...
    private GraphPathSearch<TopologyVertex, TopologyEdge> graphPathSearch() {
        return defaultGraphPathSearch != null ? defaultGraphPathSearch
                : DIJKSTRA;
    }

    private LinkWeigher linkWeight() {
        return defaultLinkWeigher != null ? defaultLinkWeigher
                : hopCountWeigher;
    }
    ...
    private static class NoIndirectLinksWeigher
        ...
        public Weight weight(TopologyEdge edge) {
            return edge.link().state() == INACTIVE
                    || edge.link().type() == INDIRECT ? getNonViableWeight()
    ...
    public String toString() {
        return toStringHelper(this).add("time", time)
                .add("creationTime", creationTime)
                .add("computeCost", computeCost).add("clusters", clusterCount())
                .add("devices", deviceCount()).add("links", linkCount())
                .toString();
    }
