...
public class ListenerRegistry<E extends Event, L extends EventListener<E>>
    ...
    public void process(E event) {
        ProvHook.recordDispatch(event);
        for (L listener : listeners) {
            try {
                lastListener = listener;
                lastStart = System.currentTimeMillis();
                if (listener.isRelevant(event)) {
                    ProvHook.recordListen(event, listener.getClass());
        ...
        if (lastStart > 0) {
            ...
            if (duration > LIMIT) {
                log.error(
                        ...
                        lastListener.getClass().getName(), duration);
    ...
    protected void reportProblem(E event, Throwable error) {
        log.warn("Exception encountered while processing event " + event,
                error);
    }
