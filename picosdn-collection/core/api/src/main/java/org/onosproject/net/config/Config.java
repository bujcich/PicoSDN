 ...
 * classes should keep all state in the specified JSON tree as that is the only
 * state that will be distributed or persisted; this class is merely a facade
 * for interacting with a particular facet of configuration on a given subject.
 *
 * @param <S> type of subject
 */
@Beta
public abstract class Config<S> implements ProvEntity {
     ...
     */
    public final void init(S subject, String key, JsonNode node,
            ObjectMapper mapper, ConfigApplyDelegate delegate) {
     ...
     * <p>
     * Default implementation returns true. Subclasses are expected to override
     * this with their own validation. Implementations are free to throw a
     * RuntimeException if data is invalid.
     * </p>
     *
    ...
    /**
     * Sets the specified property as a boolean or clears it if null value
     * given.
     *
     * @param name  property name
     * @param value new value or null to clear the property
    ...
    /**
     * Sets the specified property as an integer or clears it if null value
     * given.
     *
     * @param name  property name
     * @param value new value or null to clear the property
    ...
    protected <E extends Enum<E>> E get(String name, E defaultValue,
        ...
        if (defaultValue != null) {
            return Enum.valueOf(enumClass,
                    object.path(name).asText(defaultValue.toString()));
        }
     ...
     * <p>
     * If the {@code node} was a text node, text is returned as-is, all other
     * node type will be converted to String by toString().
     *
    ...
    protected <T> Config<S> setList(String name,
            Function<? super T, String> function, List<T> value) {
        Collection<String> mapped = value.stream().map(function)
    ...
    /**
     * Indicates whether only the specified fields are present in the backing
     * JSON.
     *
    ...
    /**
     * Indicates whether all specified fields are present in a particular JSON
     * object.
     *
     * @param node            node whose fields to check
     * @param mandatoryFields mandatory field names
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isMacAddress(ObjectNode objectNode, String field,
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isIpAddress(ObjectNode objectNode, String field,
            FieldPresence presence) {
        return isValid(objectNode, field, presence, n -> {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isIpPrefix(ObjectNode objectNode, String field,
            FieldPresence presence) {
        return isValid(objectNode, field, presence, n -> {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isTpPort(ObjectNode objectNode, String field,
            FieldPresence presence) {
        return isValid(objectNode, field, presence, n -> {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isConnectPoint(ObjectNode objectNode, String field,
            FieldPresence presence) {
        return isValid(objectNode, field, presence, n -> {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isString(String field, FieldPresence presence,
            String... pattern) {
    ...
    protected boolean isString(ObjectNode objectNode, String field,
        ...
        return isValid(objectNode, field, presence, (node) -> {
            if (!(node.isTextual()
                    && (pattern.length > 0 && node.asText().matches(pattern[0])
                            || pattern.length < 1))) {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isNumber(String field, FieldPresence presence,
            long... minMax) {
    ...
    protected boolean isNumber(ObjectNode objectNode, String field,
        ...
        return isValid(objectNode, field, presence, n -> {
            long number = n.isNumber() ? n.asLong()
                    : Long.parseLong(n.asText());
            if (minMax.length > 1) {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isIntegralNumber(String field, FieldPresence presence,
            long... minMax) {
    ...
    protected boolean isIntegralNumber(ObjectNode objectNode, String field,
            FieldPresence presence, long... minMax) {
        return isValid(objectNode, field, presence, n -> {
            long number = n.isIntegralNumber() ? n.asLong()
                    : Long.parseLong(n.asText());
            if (minMax.length > 1) {
                verifyRange(number, minMax[0], minMax[1]);
            } else if (minMax.length > 0) {
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isDecimal(String field, FieldPresence presence,
            double... minMax) {
    ...
    protected boolean isDecimal(ObjectNode objectNode, String field,
        ...
        return isValid(objectNode, field, presence, n -> {
            double number = n.isDouble() ? n.asDouble()
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    protected boolean isBoolean(ObjectNode objectNode, String field,
            FieldPresence presence) {
        return isValid(objectNode, field, presence, n -> {
            if (!(n.isBoolean()
                    || n.isTextual() && isBooleanString(n.asText()))) {
    ...
    private boolean isBooleanString(String str) {
        return str.equalsIgnoreCase(TRUE_LITERAL)
                || str.equalsIgnoreCase(FALSE_LITERAL);
    }
     ...
     * @throws InvalidFieldException if the field is present but not valid
     */
    private boolean isValid(ObjectNode objectNode, String field,
            FieldPresence presence,
