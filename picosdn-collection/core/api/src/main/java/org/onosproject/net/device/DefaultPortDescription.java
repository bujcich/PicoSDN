...
public class DefaultPortDescription extends AbstractDescription
    ...
    /**
     * Creates a DEFAULT_SPEED COPPER port description using the supplied
     * information.
     *
    ...
    @Deprecated
    public DefaultPortDescription(PortNumber number, boolean isEnabled,
            Type type, long portSpeed, SparseAnnotations... annotations) {
    ...
    public DefaultPortDescription(PortNumber number, boolean isEnabled,
            boolean isRemoved, Type type, long portSpeed,
            SparseAnnotations... annotations) {
        super(annotations);
    ...
    public DefaultPortDescription(PortDescription base,
            SparseAnnotations annotations) {
        this(base.portNumber(), base.isEnabled(), base.isRemoved(), base.type(),
                base.portSpeed(), annotations);
    }
     ...
     * @param annotations to be used in the copied description. Note:
     *                    Annotations on {@code base} will be ignored.
     * @return copied port description
     *
     * @deprecated in 1.13.0 use {@link #builder(PortDescription)} instead.
     */
    @Deprecated
    public static DefaultPortDescription copyReplacingAnnotation(
            PortDescription base, SparseAnnotations annotations) {
        return DefaultPortDescription.builder(base).annotations(annotations)
                .build();
    }
    ...
    public String toString() {
        return MoreObjects.toStringHelper(getClass()).add("number", number)
                .add("isEnabled", isEnabled).add("isRemoved", isRemoved)
                .add("type", type).add("portSpeed", portSpeed)
    ...
    /**
     * Creates port description builder inheriting with default parameters, from
     * specified port description.
     *
    ...
    public static class Builder {
        ...
        public DefaultPortDescription build() {
            return new DefaultPortDescription(number, isEnabled, isRemoved,
                    type, portSpeed, annotations);
        }
    }
}
