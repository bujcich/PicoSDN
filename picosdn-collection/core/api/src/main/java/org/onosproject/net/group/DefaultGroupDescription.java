...
public class DefaultGroupDescription implements GroupDescription {
    ...
    /**
     * Constructor to be used by north bound applications. NOTE: The caller of
     * this subsystem MUST ensure the appCookie provided in this API is
     * immutable. NOTE: The caller may choose to pass in 'null' for the groupId.
     * This is the typical case, where the caller allows the group subsystem to
     * choose the groupId in a globally unique way. If the caller passes in the
     * groupId, the caller MUST ensure that the id is globally unique (not just
     * unique per device).
     *
     * @param deviceId  device identifier
     * @param type      type of the group
     * @param buckets   immutable list of group bucket
     * @param appCookie immutable application cookie of type DefaultGroupKey to
     *                  be associated with the group
     * @param groupId   group identifier
     * @param appId     application id
     */
    public DefaultGroupDescription(DeviceId deviceId,
            GroupDescription.Type type, GroupBuckets buckets,
        ...
        if (this.type == GroupDescription.Type.INDIRECT) {
            checkArgument(buckets.buckets().size() == 1,
                    "Indirect group " + "should have only one action bucket");
        }
    ...
    /**
     * Constructor to be used by group subsystem internal components. Creates
     * group description object from another object of same type.
     *
    ...
    /**
     * Constructor to be used by group subsystem internal components. Creates
     * group description object from the information retrieved from data plane.
     *
     * @param deviceId device identifier
     * @param type     type of the group
     * @param buckets  immutable list of group bucket
     ...
     *         passed in null during GroupDescription creation.
     */
    @Override
    ...
    public String toString() {
        return toStringHelper(this).add("deviceId", deviceId).add("type", type)
                .add("buckets", buckets).add("appId", appId)
                .add("appCookie", appCookie).add("givenGroupId", givenGroupId)
                .toString();
    }
