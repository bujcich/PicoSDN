...
public final class DefaultRegion extends AbstractAnnotated implements Region {
     ...
     * @param masters list of sets of cluster node identifiers; in order of
     *                mastership
     */
    public DefaultRegion(RegionId id, String name, Type type,
        ...
        this.name = name;
        this.type = type;
        this.masters = masters != null ? ImmutableList.copyOf(masters)
                : ImmutableList.of();
        if (name != null) {
            checkArgument(name.length() <= NAME_MAX_LENGTH,
                    "name exceeds maximum length " + NAME_MAX_LENGTH);
        }
    }
    ...
    public String toString() {
        return MoreObjects.toStringHelper(this).add("id", id).add("name", name)
                .add("type", type).add("masters", masters).toString();
    }
