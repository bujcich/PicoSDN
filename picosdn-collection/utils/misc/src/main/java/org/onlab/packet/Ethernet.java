...
public class Ethernet extends BasePacket {
    ...
    public static final short TYPE_ARP = EthType.EtherType.ARP.ethType()
            .toShort();
    public static final short TYPE_RARP = EthType.EtherType.RARP.ethType()
            .toShort();
    public static final short TYPE_IPV4 = EthType.EtherType.IPV4.ethType()
            .toShort();
    public static final short TYPE_IPV6 = EthType.EtherType.IPV6.ethType()
            .toShort();
    public static final short TYPE_LLDP = EthType.EtherType.LLDP.ethType()
            .toShort();
    public static final short TYPE_VLAN = EthType.EtherType.VLAN.ethType()
            .toShort();
    public static final short TYPE_QINQ = EthType.EtherType.QINQ.ethType()
            .toShort();
    public static final short TYPE_BSN = EthType.EtherType.BDDP.ethType()
            .toShort();

    public static final short MPLS_UNICAST = EthType.EtherType.MPLS_UNICAST
            .ethType().toShort();
    public static final short MPLS_MULTICAST = EthType.EtherType.MPLS_MULTICAST
            .ethType().toShort();
    ...
    static {
        ImmutableMap.Builder<Short, Deserializer<? extends IPacket>> builder = ImmutableMap
                .builder();

        for (EthType.EtherType ethType : EthType.EtherType.values()) {
            if (ethType.deserializer() != null) {
                builder.put(ethType.ethType().toShort(),
    ...
    public byte[] serialize() {
        ...
        if (this.qinqVID != Ethernet.VLAN_UNTAGGED) {
            ...
            bb.putShort((short) (this.qInQPriorityCode << 13
                    | this.qinqVID & 0x0fff));
        }
        if (this.vlanID != Ethernet.VLAN_UNTAGGED) {
            bb.putShort(TYPE_VLAN);
            bb.putShort(
                    (short) (this.priorityCode << 13 | this.vlanID & 0x0fff));
        }
    ...
    public static boolean isMACAddress(final String macAddress) {
        ...
        for (int i = 0; i < 6; ++i) {
            if (Ethernet.HEXES
                    .indexOf(macBytes[i].toUpperCase().charAt(0)) == -1
                    || Ethernet.HEXES.indexOf(
                            macBytes[i].toUpperCase().charAt(1)) == -1) {
                return false;
            }
        }
        return true;
     ...
     *
     * @param macAddress The MAC address to convert into a byte array
    ...
    public String toString() {
        ...
        if (pkt instanceof ARP) {
            ...
            /*
             * When we don't know the protocol, we print using the well known
             * hex format instead of a decimal value.
             */
        ...
        if (this.getQinQVID() != Ethernet.VLAN_UNTAGGED) {
            sb.append(", dl_qinqVlan: ");
            sb.append(this.getQinQVID());
            sb.append(", dl_qinqVlan_pcp: ");
            ...
            if (payload != null && payload instanceof ICMP6) {
                ...
                if (payload != null) {
                    if (payload instanceof NeighborSolicitation) {
                        ...
                        sb.append(", ns_target_addr: ");
                        sb.append(Ip6Address.valueOf(ns.getTargetAddress())
                                .toString());
                        ...
                        sb.append(", na_target_addr: ");
                        sb.append(Ip6Address.valueOf(na.getTargetAddress())
                                .toString());
                        sb.append(", na_solicited_flag: ");
                        sb.append(na.getSolicitedFlag());
                        sb.append(", na_router_flag: ");
                        sb.append(na.getRouterFlag());
                        sb.append(", na_override_flag: ");
                        ...
                        sb.append(", rd_target_addr: ");
                        sb.append(Ip6Address.valueOf(rd.getTargetAddress())
                                .toString());
    ...
    public static Deserializer<Ethernet> deserializer() {
        return (data, offset, length) -> {
            ...
            if (ethType == TYPE_QINQ) {
                ...
                checkHeaderLength(length, ETHERNET_HEADER_LENGTH
                        + VLAN_HEADER_LENGTH + VLAN_HEADER_LENGTH);
            ...
            if (ethType == TYPE_VLAN) {
                checkHeaderLength(length,
                        ETHERNET_HEADER_LENGTH + VLAN_HEADER_LENGTH);
                final short tci = bb.getShort();
                ...
                if (ethType == TYPE_VLAN) {
                    // We handle only double tagged packets here and assume that
                    // in this case
    ...
    public Ethernet duplicate() {
        try {
            ...
            // If we can't make an object out of the serialized data, its a
            // defect
